...
def main():
    ...
    envs = make_vec_envs(expert_st, expert_ur, args.seed, args.num_processes, args.gamma, device)

    rollouts = RolloutStorage(args.num_steps, args.num_processes, STATE_DIM*5, USER_DIM, ACTION_DIM)
    ...
    for j in range(num_updates):
        ...
        for step in range(args.num_steps):
            ...
            if action.item() != 9:
                ...
                if obs != None:
                    rollouts.insert(obs, rollouts.user[step], action, action_log_prob, value)
        ...
        if (j % args.save_interval == 0
            ...
            save_path = os.path.join(args.save_dir, str(args.lr), str(args.gail_batch_size), "entropy_" + str(args.entropy_coef), "D_lr"+str(args.D_lr))
            try:
                os.makedirs(save_path)
            except OSError:
                pass

            torch.save(actor_critic, os.path.join(save_path,"ac_{}.pt".format(j)))
